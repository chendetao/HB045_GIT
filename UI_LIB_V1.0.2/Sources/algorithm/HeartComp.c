...
unsigned char heart_get_comp_val(void)
{
    ...
    if ( comp_hr < 128 )
    {
      ...
      {
        comp_hr += randTable[rand]%3; 
      }else if ( factor < (float)0.10000 ){     
        comp_hr += randTable[rand]%5; 
      }else{
        comp_hr += randTable[rand]%7; 
      }
    }else
    {
      comp_hr = 128 + randTable2[rand]%9; 
    }
...
const unsigned char spo2_table[] = {2,2,2,3,3,3,0,4,5,4,4,5,6,6,5,6,7,8,7,9,1,3,
	1,1,2,2,3,1,3,4,5,4,4,5,9,6,5,6,7,8,7,9,1,0};
int spo2_table_idx = 0;
	
float heart_get_comp_spo2( int hr)
{
	float val = 98.0f;

	val += spo2_table[spo2_table_idx] * 0.1f;
	spo2_table_idx++;
