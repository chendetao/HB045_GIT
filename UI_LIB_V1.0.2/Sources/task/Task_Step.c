...
// struct bma2x2_accel_data_fifo accel_fifo_temp[FIFO_DEPTH*2];
...
void put_sleep(unsigned char *buf)
{
	unsigned long utc;
	struct tm time;
	...
	time.tm_year = BCDToDEC(buf[0])+2000 - 1900;//TimerYY()+2000 - 1900;
	time.tm_mon  = BCDToDEC(buf[1])-1;//TimerMM()-1;
	time.tm_mday = BCDToDEC(buf[2]);//TimerDD();
	time.tm_hour = BCDToDEC(buf[3]);//TimerHH();
	time.tm_min  = BCDToDEC(buf[4]);//TimerMI();
	time.tm_sec  = 0;//TimerSE();
	
	utc = mktime(&time);	
	
	if ( AutoSleep.start_flag == 0 )
	{
		AutoSleep.start_time_utc = utc;
		AutoSleep.start_flag = 1;
	}
	
	memset(&time, 0, sizeof(time));
	
	time.tm_year = BCDToDEC(buf[5])+2000 - 1900;//TimerYY()+2000 - 1900;	
	time.tm_mon  = BCDToDEC(buf[6])-1;//TimerMM()-1;	
	time.tm_mday = BCDToDEC(buf[7]);//TimerDD();	
	time.tm_hour = BCDToDEC(buf[8]);//TimerHH();
	time.tm_min  = BCDToDEC(buf[9]);//TimerMI();
	time.tm_sec  = 0;//TimerSE();
	
	utc = mktime(&time);
	AutoSleep.end_time_utc = utc;
	
	flash_temp_queue_put(FM_ID_SLEEP, buf, 16);	
}
...
unsigned char takePhotoEnable = 1;
...
unsigned long taskStep( unsigned long task_id, unsigned long events )
{
	...
	{ 
		...
		// memcpy(accel_fifo_temp+16,accel_fifo,sizeof(accel_fifo));
		
		#if 0
		int r = accel_fifo[0].x, rc = 0;
		...
		// memcpy(accel_fifo_temp,accel_fifo,sizeof(accel_fifo));
		...
		{ 
			...
			/* Turn over the wrist */
			if ( /* abs(sum_y) > 180 || */ sum_x < -250 && sum_x > -300) 
			{
		...
		{
			if ( (win_current == WIN_ID_TAKE_PHOTO) && (config.lcdEnable == 1)
				&& (config.control & BSTOTW_FLAG_TURN_OVER_ENABLE)
				&& ( takePhotoEnable == 1 )/* && (rc > 9) */)
			{
				takePhotoEnable = 0;
				ui_window_manager( WINDOW_SWITCH, 0, 0);
				
				config.control &= ~BSTOTW_FLAG_TURN_OVER_ENABLE;
				
				osal_start_timerEx( task_id, TASK_STEP_TAKE_ENABLE_EVT, 2000);
			}
	...
	if ( events & TASK_STEP_TAKE_ENABLE_EVT )
	{
		takePhotoEnable = 1;
		
		return ( events ^ TASK_STEP_TAKE_ENABLE_EVT );
	}
    ...
    ***************************************************************************/
    ...
    if ( events & STEP_TASK_SLEEP_DETECT_TICK_EVT )
    {
        ...
        for ( int i = 0; i < AutoSleep.output_counter; i++ )
        {
			...
			// record the first data when sleep start on new day
			if ( AutoSleep.start_flag == 0 )
			{
				AutoSleep.start_flag = 1;
				AutoSleep.start_time_utc = Convert_UTC_time(&sleep_stimer);
			}
			
			// update the end time very time.
			
			AutoSleep.end_time_utc = Convert_UTC_time(&sleep_etimer);
        }
	...
	if ( events & TASK_STEP_SLEEP_STATISTICS_EVT )
	{
		...
		local_tx_buf[2] = (AutoSleep.start_time_utc>>24) & 0xFF;
		local_tx_buf[3] = (AutoSleep.start_time_utc>>16) & 0xFF;
		local_tx_buf[4] = (AutoSleep.start_time_utc>>8) & 0xFF;
		local_tx_buf[5] = (AutoSleep.start_time_utc>>0) & 0xFF;
		
		local_tx_buf[6] = (AutoSleep.end_time_utc>>24) & 0xFF;
		local_tx_buf[7] = (AutoSleep.end_time_utc>>16) & 0xFF;
		local_tx_buf[8] = (AutoSleep.end_time_utc>>8) & 0xFF;
		local_tx_buf[9]= (AutoSleep.end_time_utc>>0) & 0xFF;		
		
		local_tx_buf[10] = AutoSleep.out_TotalSleepTime>>8; 
		local_tx_buf[11] = AutoSleep.out_TotalSleepTime & 0xFF;
		
		local_tx_buf[12] = AutoSleep.out_TotalDeepTime>>8;
		local_tx_buf[13] = AutoSleep.out_TotalDeepTime & 0xFF;
		
		local_tx_buf[14] = AutoSleep.out_TotalLightTime>>8;
		local_tx_buf[15] = AutoSleep.out_TotalLightTime & 0xFF;		
		
		local_tx_buf[16] = AutoSleep.out_TotalWakeTime>>8;
		local_tx_buf[17] = AutoSleep.out_TotalWakeTime & 0xFF;	

		bt_protocol_tx(local_tx_buf, sizeof(local_tx_buf));
		
		return ( events ^ TASK_STEP_SLEEP_STATISTICS_EVT );
	}
    ...
    if ( events & STEP_TASK_CLEAR_SLEEP_EVT )
    {
		...
		AutoSleep.start_flag = 0x0;
		AutoSleep.start_time_utc = 0;
		AutoSleep.end_time_utc = 0;
...
void taskStepInit( unsigned long task_id )
{
	...
	osal_start_timerEx(task_id, TASK_STEP_BMA_INIT_EVT, 200);
}
