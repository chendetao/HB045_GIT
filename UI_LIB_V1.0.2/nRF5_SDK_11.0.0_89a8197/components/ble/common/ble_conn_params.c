...
static unsigned long m_conn_params_timer_id;
...
uint32_t ble_conn_params_init(const ble_conn_params_init_t * p_init)
{
    ...
    // #warning "implement your timer code on here...."
	
    ret = osal_create_timer(&m_conn_params_timer_id, 0, update_timeout_handler );
    if ( ret != 0 )
    {
...
uint32_t ble_conn_params_stop(void)
{
    ...
    // ʹԃϒćؔܺք֨ʱǷ࠲ݜ4պͦԭӧ͡٩ք֨ʱǷگ˽
    // #warning "implement your timer code on here...."
    
    osal_stop_timer(m_conn_params_timer_id);
...
static void conn_params_negotiation(void)
{
    ...
    if (!is_conn_params_ok(&m_current_conn_params))
    {
        ...
        else
        {
            ...
            // ʹԃϒćؔܺք֨ʱǷ࠲ݜ4պͦԭӧ͡٩ք֨ʱǷگ˽
            // #warning "implement your timer code on here...."
            ret = osal_start_timer(m_conn_params_timer_id, timeout_ticks );  
...
static void on_disconnect(ble_evt_t * p_ble_evt)
{
    ...
    ret = osal_stop_timer( m_conn_params_timer_id );  
