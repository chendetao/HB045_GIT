...
void ui_calling_notify( int msg_id, unsigned long arg)
{
	...
	if ( (++ccflag %2 ) == 0){ 
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_call,0X66C3);
	}else
	{    
		if ( mq.title_len != 0 )
		{
...
void DrawIcon(void)
{
	...
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_sms,0X072E);
	}else if ( mq.type == NOTIFY_TYPE_QQ )
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_qq, 0X4D7D);
	}else if ( mq.type == NOTIFY_TYPE_WX )
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_wx,0X863D);
	}else if ( mq.type == NOTIFY_TYPE_WHATSAPP )
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_wa_1,0X258F);
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_wa_2,0xFFFF);
	}else if ( mq.type == NOTIFY_TYPE_FACEBOOK )
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_fb_1,0X3AD3);
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_fb_2,0xFFFF);
...
{  
	...
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_ble_lose_1,0x1F<<00);
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_ble_lose_2,0x1F<<11);		
	}
...
{  
	...
	if ( config.showFlag++ & 0x1 )
	{
		DrawUI_Bitmap_Ex( (80-72)/2, (160-72)/2, 72, 72, BMP_sedentary, 0XFCE0);	
	}
...
static const int notify_max[7] =
{
    ...
    30, /* ALARM 15s */                  // 15
    14, /* FINDME 7s */   
    4, /* BLE Disconnnected 2s */        // 2
    8, /* Sedentary notification 4s */   // 4  
};
    ...
    {                
        ...
        if ( win_ticks++ < notify_max[notify_id-1] )
        {
           ...
           
            {
				...
				{
					win_ticks = 0;
					// osal_start_timerEx( taskGUITaskId, TASK_GUI_UPDATE_EVT, 200);
					osal_set_event( taskGUITaskId, TASK_GUI_UPDATE_EVT );
            ...
            {				
                ...
                if ( ((flag >>7) & 0x1 ) != 1 )
                {
                  ...
                  }else
                  {
					  /* 继续更新屏幕,显示内容 */
					  win_ticks = 0;
					  if ( MessageUpdatePage() == true ) // 翻页显示
					  {
