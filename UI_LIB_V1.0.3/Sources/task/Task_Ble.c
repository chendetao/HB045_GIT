...
// #define DEVICE_NAME                      "F07"                                      /**< Name of device. Will be included in the advertising data. */
...
static void gap_params_init(void)
{
	...
	#if 0   // ä½¿ç”¨Flashçš„åå­—æ¥ä½œä¸ºè“ç‰™åç§°
                                           ...
                                           ret_code_t        event_result)
{
    ...
    switch (p_event->event_id)
    {
		...
		#else
			osal_start_timerEx ( taskBleTaskId, ANCS_DISCOVERY_DELAY_START_EVT, 1000);		
		#endif
            break; 
...
unsigned long taskBle( unsigned long task_id, unsigned long events )
{
	...
	if ( events & TASK_BLE_STACK_INIT_EVT )
	{
		/** To initialize the BLE stack */
		
		// osal_set_event ( task_id, TASK_BLE_MODIFY_DEVICE_NAME_EVT );	
				
		osal_start_timerEx(task_id, TASK_BLE_INIT_EVT, 250);
		...
		return  ( events ^ TASK_BLE_STACK_INIT_EVT );
	}
...
void taskBleInit( unsigned long task_id )
{
    ...
    osal_start_timerEx(task_id, TASK_BLE_STACK_INIT_EVT, 250);
    #endif
}
