...
//	AutoSleep.out_TotalSleepTime = fm.autoSleepTime;
	...
	strncpy(config.device_name, fm.device_name, DEVICE_NAME_LENGTH+1);
	
#if (OTA_MODE_ENABLE==0)	
	config.test_mdoe = fm.test_mode;
#endif
}
...
void store_config_default(void)
{
	...
	// 默认名字
	strncpy(fm.device_name, DEVICE_NAME_CUSTOM, strlen(DEVICE_NAME_CUSTOM));
...
//	fm.autoSleepTime = AutoSleep.out_TotalSleepTime;
	...
	strncpy(fm.device_name, config.device_name, DEVICE_NAME_LENGTH+1);
	
#if (OTA_MODE_ENABLE==0)	
	fm.test_mode = config.test_mdoe;
#endif
}
...
int flash_memory_Init(void)
{
	...
	if ( ((struct FlashMemory*)swap)->control_magic != 0x8A7B43C3 )
	{
		...
		strncpy(config.device_name, DEVICE_NAME_CUSTOM, strlen(DEVICE_NAME_CUSTOM));
...
{ 
	...
	{
        if ( flash_memory_control_update() != 0 )
        {
            osal_start_timerEx( task_id, TASK_FLASH_UPDATE_EVT, 1000);		
        }
	...
	if ( events & TASK_FLASH_PUT_TEST_EVT )
	{
		...
		// flash_memory_put(FM_ID_HR, buf,16);
	...
	if ( events & TASK_FLASH_PUT_BP_EVT )
	{
		...
		// flash_memory_put(FM_ID_B_PRESSURE, buf,16);
	...
	if ( events & TASK_FLASH_QUEUE_PUT_EVT )
	{
		...
		if ( h != t )
		{
			if ( flash_memory_put( FM_ID_SLEEP, f_buf[h], 16 ) == 0 )
			{
