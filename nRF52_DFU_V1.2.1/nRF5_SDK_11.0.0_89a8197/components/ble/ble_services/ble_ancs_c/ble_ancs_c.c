...
bool ble_ancs_perform_notification_action(ble_ancs_c_t       * p_ancs, uint8_t uid[], bool action)
{
	tx_message_t * p_msg;
	
	if ( p_ancs->conn_handle == 0xFFFF )
	{
		 return (false);
	}
	...
	p_msg->req.write_req.gattc_value[index++] = BLE_ANCS_COMMAND_ID_GET_PERFORM_NOTIF_ACTION; // Command ID
	p_msg->req.write_req.gattc_value[index++] = uid[0];
	p_msg->req.write_req.gattc_value[index++] = uid[1];
	p_msg->req.write_req.gattc_value[index++] = uid[2];
	p_msg->req.write_req.gattc_value[index++] = uid[3];
	p_msg->req.write_req.gattc_value[index++] = (action==false)?(1):(0);
